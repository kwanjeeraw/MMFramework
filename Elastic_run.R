...
filenamepath = paste0(outpath,"/",method,"_elastic_");
################## Setup input ##################
...
## 1. Elastic Net model
set.seed(14)
fitControl = trainControl(method = "repeatedcv", number = 3, repeats = 5, classProbs = TRUE, summaryFunction = multiClassSummary, savePredictions = TRUE) #Repeated 3-fold CV; 70/30 for model optimization
enet_grid = expand.grid(alpha = seq(0.1, 0.9, by = 0.1), lambda = seq(0.01, 0.2, length = 5))
...
for(i in 1:length(TrainList)){
  ...
  if(nlevels(mbobj_train$Y) > 2){#multi-class
    predmodel = train(Class ~ ., data = inModeling, method = "glmnet", trControl = fitControl, preProc = NULL, family = "multinomial",
                      tuneGrid = enet_grid, metric = "Accuracy") #glmnet
  }else{#binary class
    predmodel = train(Class ~ ., data = inModeling, method = "glmnet", trControl = fitControl, preProc = NULL,
                      tuneGrid = enet_grid, metric = "Accuracy") #glmnet
  }
  ...
  if(nlevels(mbobj_train$Y) > 2){#multi-class
    ...
    imp_class = varImp(predmodel, scale = FALSE)$importance
  ...
  imp_raw = rowMeans(varImp(predmodel, scale = FALSE)$importance) #coefficient
