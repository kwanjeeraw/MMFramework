...
from tensorflow.keras.layers import Input, Conv2D, MaxPooling2D, GlobalAveragePooling2D, Dense, ReLU
...
# -----------------------------
parser = argparse.ArgumentParser(description="Run CNN Grad-CAM")
...
filenamepath = f"{outpath}/{pmethod}_gradcam_"
CNN_model = {}
CNN_model_per_class = {}
CNN_eval = {}
CNN_feature = {}
...
for m in range(len(trainlist_dict)):
    ...
    y_train_unique_vals, y_train_first_indices = np.unique(y_train_enc, return_index=True)
    y_test_unique_vals, y_test_first_indices = np.unique(y_test_enc, return_index=True)
    ...
    # -----------------------------
    # 3. Convert features to heatmaps
    # -----------------------------
    # Convert features into a heatmap of size AxA
    hmsize = math.ceil(math.sqrt(num_ft)) #a heatmap of size AxA
    X_train_img = np.array([MMFramework.features_to_heatmap(row, size=(hmsize,hmsize)) for row in X_train_scaled]) #convert train to heatmap
    X_test_img = np.array([MMFramework.features_to_heatmap(row, size=(hmsize,hmsize)) for row in X_test_scaled]) #convert test to heatmap

    # ---------------------------
    # 4. Build 2D CNN -- 2 layers
    # ---------------------------
    seed = 14
    MMFramework.set_all_seeds(seed)

    print("###Calculating CNN: ","...")
    inputs = Input(shape=(hmsize,hmsize,1))
    x = Conv2D(16, (3,3), padding='same')(inputs) #optimize
    x = ReLU()(x)
    x = MaxPooling2D((2,2))(x) #optimize
    if hmsize > 3:#features > 9
        x = Conv2D(32, (3,3), padding='same')(x) #optimize
        x = ReLU()(x)
        x = MaxPooling2D((2,2))(x) #optimize
    ...
    # 5. Train Model
    # -----------------------------
    cnn_model = model.fit(X_train_img, y_train_cat,validation_split=0.3,epochs=epochs, batch_size=batchs) #about 30% validation

    # ---------------------------
    # 6. Evaluate model
    # ---------------------------
    y_pred_prob = model.predict(X_test_img) #predicted probabilities
    ...
    # 7. Compute Grad-CAM
    # ---------------------------
    # Compute Grad-CAM feature importance on Training data
    gradcam_importance = np.zeros((len(X_train_img), num_features))
    for i in range(len(X_train_img)):
        img = X_train_img[i]
        pred_class = y_train_enc[i]
        cam = MMFramework.grad_cam(model, img, class_index=pred_class)
    
        # Map CAM back to features
        feature_importance = []
        idx = 0
        for r in range(cam.shape[0]):
            for c in range(cam.shape[1]):
                if idx < num_features:
                    feature_importance.append(cam[r, c])
                idx += 1
        gradcam_importance[i, :] = feature_importance
    
    # Average overall gradcam scores across samples on Training data
    avg_importance = gradcam_importance.mean(axis=0)
    gradcam_scores = pd.DataFrame({
        "Feature": feature_names,
        "Feature_Importance": avg_importance
    }).sort_values("Feature_Importance", ascending=False)
    
    # Average gradcam scores per class on Training data
    gradcam_df = []
    for c in range(num_classes):
        idx = np.where(y_train_enc == c)[0]
        gradcam_mean = gradcam_importance[idx].mean(axis=0)
        gradcam_df.append(gradcam_mean)
    gradcam_df = np.array(gradcam_df)
    gradcam_per_class = pd.DataFrame(gradcam_df.T,columns=ls_classes)
    gradcam_per_class.insert(0, "Feature", feature_names)
    
    # Compute Grad-CAM feature importance on Test data
    gradcam_importance_t = np.zeros((len(X_test_img), num_features))
    for i in range(len(X_test_img)):
        img = X_test_img[i]
        pred_class = np.argmax(y_pred_prob[i])
        cam = MMFramework.grad_cam(model, img, class_index=pred_class)

        # Map CAM to features
        feature_importance_t = []
        idx = 0
        for r in range(X_test_img.shape[1]):
            for c in range(X_test_img.shape[2]):
                if idx < num_features:
                    feature_importance_t.append(cam[r,c])
                idx += 1
        gradcam_importance_t[i,:] = feature_importance_t

    # Average overall gradcam scores across samples on Test data
    avg_importance_t = gradcam_importance_t.mean(axis=0)
    gradcam_scores_t = pd.DataFrame({
        "Feature": feature_names,
        "Feature_Importance": avg_importance_t
    }).sort_values("Feature_Importance", ascending=False)
    
    # Average gradcam scores per class on Test data
    gradcam_df_t = []
    for c in range(num_classes):
        idx = np.where(y_test_enc == c)[0]
        gradcam_mean_t = gradcam_importance_t[idx].mean(axis=0)
        gradcam_df_t.append(gradcam_mean_t)
    gradcam_df_t = np.array(gradcam_df_t)
    gradcam_per_class_t = pd.DataFrame(gradcam_df_t.T,columns=ls_classes)
    gradcam_per_class_t.insert(0, "Feature", feature_names)
 
    print(cnn_metrics);print("###Exporting output data: ",dataset_name)

    # ---------------------------
    # 8. Collect Grad-CAM outputs
    # ---------------------------
    ...
    CNN_feature[dataset_name] = gradcam_scores #collect gradcam_scores
    CNN_feature_per_class[dataset_name] = gradcam_per_class #collect gradcam_per_class
    ...
    with PdfPages(fig_fname) as pdf:
        #Heatmap examples
        ##MMFramework.plot_multiple_heatmaps(X_train_img[0:6], y_train[0:6], feature_names, size=(hmsize,hmsize), title="Heatmaps of Training Set")
        ##pdf.savefig(dpi=300, bbox_inches='tight')  #Save to PDF
        ##plt.close()
        ##MMFramework.plot_multiple_heatmaps(X_test_img[0:6], y_test[0:6], feature_names, size=(hmsize,hmsize), title="Heatmaps of Test Set")
        ...
        plt.close()
        #importance scores
        MMFramework.plot_importance_barplot(gradcam_scores, figsize=(7,hmsize*1.5))
        pdf.savefig(dpi=300, bbox_inches='tight')
        plt.close()
        
        ##Training set ##
        #active feature scores threshold
        MMFramework.plot_avg_gradcam_heatmap(avg_importance, feature_names, threshold=0.5, size=(hmsize,hmsize), title="All classes (training set)")
        pdf.savefig(dpi=300, bbox_inches='tight')
        plt.close()
        #active feature top n
        MMFramework.plot_avg_gradcam_heatmap_topn(avg_importance, feature_names, top_n=5, size=(hmsize,hmsize), title="All classes (training set)")
        pdf.savefig(dpi=300, bbox_inches='tight')
        plt.close()
        #active feature scores threshold
        MMFramework.plot_avg_heatmap_active_cells(X_train_scaled, y_train, avg_importance, feature_names, threshold=0.5, size=(hmsize,hmsize), title="- (training set)")
        pdf.savefig(dpi=300, bbox_inches='tight')
        plt.close()
        #active feature top n
        MMFramework.plot_avg_heatmap_active_cells_topn(X_train_scaled, y_train, avg_importance, feature_names, top_n=5, size=(hmsize,hmsize), title="- (training set)")
        pdf.savefig(dpi=300, bbox_inches='tight')
        plt.close()
        ##Test set ##
        #active feature scores threshold
        MMFramework.plot_avg_gradcam_heatmap(avg_importance_t, feature_names, threshold=0.5, size=(hmsize,hmsize), title="All classes (test set)")
        pdf.savefig(dpi=300, bbox_inches='tight')
        plt.close()
        #active feature top n
        MMFramework.plot_avg_gradcam_heatmap_topn(avg_importance_t, feature_names, top_n=5, size=(hmsize,hmsize), title="All classes (test set)")
        pdf.savefig(dpi=300, bbox_inches='tight')
        plt.close()
        #active feature scores threshold
        MMFramework.plot_avg_heatmap_active_cells(X_test_scaled, y_test, avg_importance_t, feature_names, threshold=0.5, size=(hmsize,hmsize), title="- (test set)")
        pdf.savefig(dpi=300, bbox_inches='tight')
        plt.close()
        #active feature top n
        MMFramework.plot_avg_heatmap_active_cells_topn(X_test_scaled, y_test, avg_importance_t, feature_names, top_n=5, size=(hmsize,hmsize), title="- (test set)")
        pdf.savefig(dpi=300, bbox_inches='tight')
        plt.close()
        ...
        plt.close()
        #feature importance
        df_imp = gradcam_per_class.set_index("Feature")[ls_classes]
        ...
        for cls in ls_classes:
            ...
            importance_per_class = gradcam_per_class[cls].values
            #active feature scores threshold
            MMFramework.plot_avg_gradcam_heatmap(importance_per_class, feature_names, threshold=0.5, size=(hmsize,hmsize), title=f"{cls} (training set)")
            pdf.savefig(dpi=300, bbox_inches='tight')
            plt.close()
            #active feature top n
            MMFramework.plot_avg_gradcam_heatmap_topn(importance_per_class, feature_names, top_n=5, size=(hmsize,hmsize), title=f"{cls} (training set)")
            pdf.savefig(dpi=300, bbox_inches='tight')
            plt.close()
            ##Test set ##
            # Get importance values for this class
            importance_per_class_t = gradcam_per_class_t[cls].values
            #active feature scores threshold
            MMFramework.plot_avg_gradcam_heatmap(importance_per_class_t, feature_names, threshold=0.5, size=(hmsize,hmsize), title=f"{cls} (test set)")
            pdf.savefig(dpi=300, bbox_inches='tight')
            plt.close()
            #active feature top n
            MMFramework.plot_avg_gradcam_heatmap_topn(importance_per_class_t, feature_names, top_n=5, size=(hmsize,hmsize), title=f"{cls} (test set)")
            pdf.savefig(dpi=300, bbox_inches='tight')
            plt.close()
        ##Training set ##
        #active feature scores threshold per class
        MMFramework.plot_avg_heatmap_active_cells_per_class(X_train_scaled, y_train, gradcam_per_class, feature_names, threshold=0.5, ncol=2, size=(hmsize,hmsize), title="- training set)")
        pdf.savefig(dpi=300, bbox_inches='tight')
        plt.close()
        #active feature top n per class
        MMFramework.plot_avg_heatmap_active_cells_topn_per_class(X_train_scaled, y_train, gradcam_per_class, feature_names, top_n=5, ncol=2, size=(hmsize,hmsize), title="- training set)")
        pdf.savefig(dpi=300, bbox_inches='tight')
        plt.close()
        ##Test set ##
        #active feature scores threshold per class
        MMFramework.plot_avg_heatmap_active_cells_per_class(X_test_scaled, y_test, gradcam_per_class_t, feature_names, threshold=0.5, ncol=2, size=(hmsize,hmsize), title="- test set)")
        pdf.savefig(dpi=300, bbox_inches='tight')
        plt.close()
        #active feature top n per class
        MMFramework.plot_avg_heatmap_active_cells_topn_per_class(X_test_scaled, y_test, gradcam_per_class_t, feature_names, top_n=5, ncol=2, size=(hmsize,hmsize), title="- test set)")
        pdf.savefig(dpi=300, bbox_inches='tight')
        plt.close()
